...
namespace TestFirstProject.Configurations
{
    public class PaymentAttemptConfiguration : IEntityTypeConfiguration<PaymentAttempt>
    {
        public void Configure(EntityTypeBuilder<PaymentAttempt> builder)
        {
            ...
            builder.Property(a => a.FailureReason).HasColumnName("failure_reason").HasMaxLength(500);
            builder.Property(a => a.GatewayResponse).HasColumnName("gateway_response").HasMaxLength(2000);
            builder.Property(a => a.AttemptedAt).HasColumnName("attempted_at");

            builder.HasOne(a => a.Payment).WithMany(p => p.Attempts).HasForeignKey(a => a.PaymentId).OnDelete(DeleteBehavior.Cascade);
